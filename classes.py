...
class Hero(pygame.sprite.Sprite):
    def __init__(self, player_group, all_sprites, classes, pos_x,
        ...
        self.defence = (self.cur.execute(f"""SELECT defence
                            FROM classes
                            WHERE name = '{classes}'""").fetchall())[0][0]
        # процент наносимого урона
        self.float_defence = (101 - self.defence) / 100
        self.speed = (self.cur.execute(f"""SELECT speed
                            FROM classes
                            WHERE name = '{classes}'""").fetchall())[0][0]
        self.intell = (self.cur.execute(f"""SELECT intelligence
                            FROM classes
                            WHERE name = '{classes}'""").fetchall())[0][0]
        ...
        if level_window:
            if classes == 'wizard':
                ...
                self.move_animation = [load_image.loadimage("wizard/1.png"),
                                       load_image.loadimage("wizard/2.png")]
            elif classes == 'archer':
                self.image = load_image.loadimage("archer/0.png")
                self.move_animation = [load_image.loadimage("archer/1.png"),
                                       load_image.loadimage("archer/2.png")]
            elif classes == 'barbarian':
                self.image = load_image.loadimage("barbarian/0.png")
                self.move_animation = [load_image.loadimage("barbarian/1.png"),
                                       load_image.loadimage("barbarian/2.png")]
            elif classes == 'crusader':
                self.image = load_image.loadimage("crusader/0.png")
                self.move_animation = [load_image.loadimage("crusader/1.png"),
                                       load_image.loadimage("crusader/2.png")]
            elif classes == 'necromancer':
                self.image = load_image.loadimage("necro/0.png")
                self.move_animation = [load_image.loadimage("necro/1.png"),
                                       load_image.loadimage("necro/2.png")]
