    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and pygame.mouse.get_pressed():
            ...
            if 200 <= event.pos[0] <= 400 and 200 <= event.pos[1] <= 250\
                ...
                # если сохранения нет, то открывается окно игрока
                    ...
                    money_number = (curr.execute("""SELECT money
                                FROM player
                                WHERE key = 1""").fetchall())[0][0]
                    text_money = font.render(str(money_number), 1,
                                             (0, 0, 0), BACKGROUND_TEXT_MONEY)
                    pygame.draw.rect(screen, BACKGROUND_TEXT_MONEY,
                                     ((390, 20), text_money.get_size()))
                    screen.blit(text_money, (390, 20))
                    Flag_start_window_on = False
            # кнопка "Да" в диалоговом окне (стирается сохранение)
            if 180 <= event.pos[0] <= 280 and 350 <= event.pos[1] <= 380\
                    and Flag_dialog_window_on:
                playerwindow = window.Window(screen)
                playerwindow.draw('player window.jpg')
            ...
            if 320 <= event.pos[0] <= 420 and 350 <= event.pos[1] <= 380\
                    and Flag_dialog_window_on:
                startwindow = window.Window(screen)
                startwindow.draw('start window.jpg')
                Flag_start_window_on = True
            ...
            if 200 <= event.pos[0] <= 400 and 300 <= event.pos[1] <= 350\
                    and save_file_exist and Flag_start_window_on:
                playerwindow = window.Window(screen)
                playerwindow.draw('player window.jpg')
                Flag_start_window_on = False
                Flag_player_window_on = True
                ...
                screen.blit(text_money, (390, 20))
            # кнопка "Правила"
            if 200 <= event.pos[0] <= 400 and 400 <= event.pos[1] <= 450\
                    and Flag_start_window_on:
            ...
            if 520 <= event.pos[0] <= 600 and 0 <= event.pos[1] <= 30\
                    and Flag_rules_window_on:
                Flag_rules_window_on = False
                Flag_start_window_on = True
                startwindow = window.Window(screen)
                startwindow.draw('start window.jpg')
            # кнопка "Сохранение"
            if 520 <= event.pos[0] <= 600 and 0 <= event.pos[1] <= 40\
                    and Flag_player_window_on:
                curr.execute(f"""UPDATE player
                                SET money = {money_number}
                                WHERE key = 1""").fetchall()

            # кнопки выбора персонажа и его просмотра
            if 100 < event.pos[1] < 160 and 20 < event.pos[0] < 250\
                    and Flag_player_window_on:
                ...
                Flag_select_crusader = False
                Flag_select_necromancer = False
            if 160 < event.pos[1] < 220 and 20 < event.pos[0] < 250\
                    and Flag_player_window_on:
                ...
                Flag_select_crusader = False
                Flag_select_necromancer = False
            if 220 < event.pos[1] < 280 and 20 < event.pos[0] < 250\
                    and Flag_player_window_on:
                ...
                Flag_select_crusader = False
                Flag_select_necromancer = False
            if 280 < event.pos[1] < 340 and 20 < event.pos[0] < 250\
                    and Flag_player_window_on:
                ...
                Flag_select_necromancer = False
            if 340 < event.pos[1] < 400 and 20 < event.pos[0] < 250\
                    and Flag_player_window_on:
            ...
            if (450 < event.pos[1] < 520 and 20 < event.pos[0] < 250 and
                Flag_player_window_on and
               (Flag_select_archer or Flag_select_wizard or
                Flag_select_necromancer or Flag_select_barbarian or
            ...
            if 540 < event.pos[1] < 580 and 40 < event.pos[0] < 230\
                    and Flag_player_window_on:
                startwindow = window.Window(screen)
                startwindow.draw('start window.jpg')
                Flag_start_window_on = True
            ...
            if 370 <= event.pos[0] <= 500 and 540 <= event.pos[1] <= 570\
                    and Flag_player_window_on:
                if Flag_select_necromancer and money_number - 100 >= 0:
                    upgrade_hero_stats = classes.Hero(player_group,
                                                      all_sprites,
                                                      'necromancer', 0, 0,
                                                      curr, False)
            ...
            if 530 < event.pos[1] < 580 and 230 < event.pos[0] < 370\
                    and Flag_level_window_on:
                playerwindow = window.Window(screen)
                playerwindow.draw('player window.jpg')
                money_number = (curr.execute("""SELECT money
                            FROM player
                ...
                Flag_player_window_on = True
            # кнопка 1 уровня
            if 250 < event.pos[1] < 350 and 50 < event.pos[0] < 150\
                    and Flag_level_window_on:
                if Flag_select_crusader:
                    hero, level_x, level_y = generate_levels.generate_level(
                        generate_levels.load_level("level 1.txt"),
            ...
            # кнопка 2 уровня
            if 250 < event.pos[1] < 350 and 250 < event.pos[0] < 350\
                    and Flag_level_window_on:
                if Flag_select_crusader:
                    hero, level_x, level_y = generate_levels.generate_level(
                        generate_levels.load_level("level 2.txt"),
                ...
                Flag_game_process_window_on = True
            # кнопка 3 уровня
            if 250 < event.pos[1] < 350 and 450 < event.pos[0] < 550\
                    and Flag_level_window_on:
                if Flag_select_crusader:
                    hero, level_x, level_y = generate_levels.generate_level(
                        generate_levels.load_level("level 3.txt"),
        ...
        if event.type == pygame.KEYDOWN and Flag_game_process_window_on:
            if (event.key == pygame.K_LEFT or event.key == pygame.K_a):
                hero.rect.x -= STEP
                answer = hero.update(tiles_group_not_collide,
                                     tiles_group_collide,
                                     health_group, money_group,
                                     exit_group)
                if not answer[0]:
                    hero.rect.x += STEP
            ...
            text_money = font.render(f'money {str(money_number)}', 1,
                                     (0, 0, 0), BACKGROUND_TEXT_MONEY)
            pygame.draw.rect(screen, BACKGROUND_TEXT_MONEY,
                             ((10, 550), text_money.get_size()))
            screen.blit(text_money, (10, 550))

            text_health = font.render(f'health {str(hero.health)}', 1,
                                      (0, 0, 0), BACKGROUND_TEXT_MONEY)
            pygame.draw.rect(screen, BACKGROUND_TEXT_MONEY,
                             ((300, 550), text_health.get_size()))
